...
Focus on micro-influencers with high engagement rates.`;
    ...
    analyzeTopPlatforms(events) {
        ...
        events.forEach(event => {
            const platforms = (event.data && event.data.platforms) ? event.data.platforms : [];
    ...
    analyzeTopCategories(events) {
        ...
        events.forEach(event => {
            const categories = (event.data && event.data.categories) ? event.data.categories : [];
    ...
    analyzeTrendEvolution(events) {
        return events.map(event => ({
            ...
            trendCount: (event.data && event.data.trends && event.data.trends.length) ? event.data.trends.length : 0,
            topTrends: (event.data && event.data.trends) ? event.data.trends.slice(0, 3) : []
        }));
    }
